...
class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        self.schedule[(key, timestamp)] = value

    def get(self, key: str, timestamp: int) -> str:
        """ This passes 44/49 tests but times out on the last 5
        """
        keys = [k for k in self.schedule.keys() if k[0] == key]
        if key in keys:
            return self.schedule[keys[keys.index(key)]]
        else:
            max_timestamp = 0
            for k,t in keys:
                if max_timestamp < t <= timestamp:
                    max_timestamp = t
            return self.schedule[(key, max_timestamp)] if max_timestamp > 0 else ""
        
    def get(self, key: str, timestamp: int) -> str:
        """ I could sort the keys and use bisect_left to 
            find the max timestamp in O(log) time
        """
        keys = sorted(self.schedule.keys)
        idx = bisect_left(keys, (key, timestamp))
        return self.schedule[keys[idx]]
